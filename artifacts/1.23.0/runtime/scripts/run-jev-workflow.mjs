...
/** @param {any} plan @param {any} context @param {string[]} [acceptanceIds] */
export function validateDriverPlan(plan, context, acceptanceIds = criterionIds) {
...
export async function runWorkflow(config, runtime = {}) {
  ...
  const planningScope = `Plan implementation packets and existing integration checks only. The controller already owns independent packet and integrated-code reviews, neutral QA using ${config.qaRecipePath}, and any configured visual critique. Do not redefine that lifecycle, replace the QA recipe, or include a second QA plan. Map acceptance criteria to implementation responsibilities; packet and integration checks never establish final acceptance.`;
  ...
  try {
    ...
    await json(resolve(directory, 'manifest.json'), { ...config, ticket, acceptanceIds, requiresVisualReview, concurrency: capacity, coordinator, startedAt: new Date().toISOString(), controllerVersion: '1.23.0' });
    await event('task-started', { ticket });
    const discovery = await success('discovery', coordinator, 'Inspect relevant code and give a concise handoff: canonical APIs/forms, dependencies, existing checks and open decisions. Read only; do not reproduce another benchmark solution.', { sandbox: 'read-only' });
    let plan = structured(await success('plan', astra, `Produce exact portable packets using this discovery and source. ${planningScope} Prefer 2–4 coherent packets; settle shared contracts before consumers. Include exact instructions, owned relative paths, dependencies and existing focused checks. Never embed this source checkout's absolute path or branch: workers use private detached worktrees. Instructions/checks must use relative paths or {{WORKTREE_ROOT}}, {{BASE_SHA}}, {{BRANCH}}; never switch branches or commit. Use exactly these acceptance IDs in recipe order and cover every criterion: ${JSON.stringify(acceptanceIds)}. Do not include benchmark/setup artifacts in product ownership.\nDISCOVERY:\n${discovery}`, { outputSchema: planSchema, sandbox: 'read-only' }));
    const context = { runId: config.arm, root, baseSha: config.baseSha, coordinator };
    for (;;) {
      try { validateDriverPlan(plan, context, acceptanceIds); }
      catch (error) {
        ...
        plan = structured(await success('plan-correction', astra, `Correct this deterministic packet contract failure: ${problem}. ${planningScope} Use only relative product paths or {{WORKTREE_ROOT}}, {{BASE_SHA}}, {{BRANCH}}. Workers use separate detached worktrees; no original-root paths, fixed branches, branch changes or commits. Preserve task coverage and exact instructions.\nPLAN:${JSON.stringify(plan)}`, { outputSchema: planSchema, sandbox: 'read-only' }));
        continue;
      }
    ...
    const workflowPlan = validateDriverPlan(plan, context, acceptanceIds);
    ...
    for (;;) {
      for (const atom of store.awaitingVerification()) {
        ...
        const prompt = `Independently inspect the immutable candidate diff and relevant code. Run relevant existing checks, do not change product files. Approve only correct observable behavior satisfying the exact packet and task.\nEXACT INSTRUCTIONS:\n${atom.instructions}\nPACKET:${JSON.stringify(atom)}\nJEV REVIEW FOCUS:${JSON.stringify(judgment)}\nPatch:${candidate.patchPath}\nBase tree:${candidate.initialTree}\nCandidate tree:${candidate.tree}\nSHA256:${candidate.hash}`;
        active.set(`review:${atom.id}`, observeAttempt(stage(`review-${atom.id}`, astra, prompt, { cwd: candidate.cwd, outputSchema: reviewSchema, sandbox: 'read-only' }),
    ...
    for (;;) {
      ...
      const review = validateReview(structured(await success('integrated-code-review', astra, `Independently review the full integrated diff against every configured task criterion ${JSON.stringify(acceptanceIds)} and actual source. No product edits. Run applicable existing checks. Inspect Git diff ${config.baseSha} to current candidate tree ${before}. Include coordinator integration/correction changes; packet approvals do not substitute for this review.`, { outputSchema: reviewSchema, sandbox: 'read-only' })));
