...
function packCompleted(run, completed, requiredDependencyIds) {
  if (!isRecord(completed)) throw new GovernanceError("completed context is unavailable");
  /** @param {string} key @returns {any[]} */
  const records = (key) => {
    const value = completed[key] ?? [];
    if (!Array.isArray(value)) throw new GovernanceError("completed records must be arrays");
    return value;
  };
  /** @param {any} entry */
  const hasId = (entry) => isRecord(entry) && typeof entry.id === "string" && entry.id.trim().length > 0;
  /** @type {Map<string, any>} */ const artifacts = new Map();
  for (const [collection, kinds] of /** @type {Array<[string, string[]]>} */ ([
    ["plans", ["plan"]], ["reviews", ["plan-review", "final-review"]], ["verifications", ["verification", "observation-assessment"]],
  ])) {
    for (const artifact of records(collection)) {
      if (!hasId(artifact) || !kinds.includes(artifact.kind) || artifacts.has(artifact.id)) throw new GovernanceError("canonical artifact identity is missing or ambiguous");
      artifacts.set(artifact.id, artifact);
    }
  }
  /** @type {Map<string, any>} */ const dependencies = new Map();
  for (const dependency of records("dependencies")) {
    if (!hasId(dependency) || dependencies.has(dependency.id)) throw new GovernanceError("dependency identity is missing or ambiguous");
    dependencies.set(dependency.id, dependency);
  }
  if (!Array.isArray(requiredDependencyIds) || new Set(requiredDependencyIds).size !== requiredDependencyIds.length) throw new GovernanceError("required dependency is missing or ambiguous");
  for (const id of requiredDependencyIds) {
    if (dependencies.has(id)) continue;
    const boundaries = (run.boundaries ?? []).filter((/** @type {any} */ entry) => entry.id === id);
    if (boundaries.length !== 1) throw new GovernanceError("required dependency is missing or ambiguous");
    const boundary = boundaries[0];
    const judgments = (run.judgments ?? []).filter((/** @type {any} */ entry) => entry.id === boundary.judgmentId);
    const judgment = judgments[0];
    if (judgments.length !== 1 || boundary.verdict !== "pass" || judgment.verdict !== "pass" || boundary.runId !== run.runId || judgment.runId !== run.runId || judgment.boundaryId !== boundary.id || typeof boundary.inputHash !== "string" || !boundary.inputHash || judgment.inputHash !== boundary.inputHash) throw new GovernanceError("required boundary dependency lacks a matching approved judgment");
    dependencies.set(id, { id, kind: "boundary", boundary, judgment });
  }
  const expectedArtifacts = new Set(requiredDependencyIds.map((id) => dependencies.get(id).acceptanceId).filter((id) => id != null));
  const returnedIds = new Set();
  const returnedArtifactRefs = records("returnedArtifacts").map((artifact) => {
    const canonical = artifacts.get(artifact?.id);
    if (!hasId(artifact) || !canonical || returnedIds.has(artifact.id) || !expectedArtifacts.has(artifact.id) || canonical.kind !== artifact.kind || stableHash(canonical) !== stableHash(artifact)) throw new GovernanceError("returned artifact is missing, ambiguous or contradictory");
    returnedIds.add(artifact.id);
    return { id: artifact.id, kind: artifact.kind };
  });
  if ([...expectedArtifacts].some((id) => !returnedIds.has(id))) throw new GovernanceError("required returned artifact is missing");
  const { returnedArtifacts, ...retained } = completed;
  return { ...retained, dependencies: [...dependencies.values()], returnedArtifactRefs, requiredDependencyIds: [...requiredDependencyIds] };
}
...
function buildState(run, proposal, sources, criteria, completed) {
  return {
    run: { runId: run.runId, root: run.root, hostRoot: run.hostRoot ?? run.root, rootIdentity: run.rootIdentity ?? null, baseSha: run.baseSha, phase: run.phase, taskKind: run.taskKind ?? "implementation", endpoint: run.endpoint, authorization: run.authorization, observedCoordinator: run.coordinator, integratedCandidatePaths: run.candidatePaths ?? [] },
    policy: {
      ...
      routeContract: "The coordinator retains the actual observed starting profile. Research uses approved fast profiles or explicitly selected Astra xhigh; planner/reviewer use Astra xhigh; writer uses explicitly selected Flash high or Astra xhigh, without automatic fallback; exact checks use local/deterministic-check. A dispatch selects a target distinct from the caller; actual target identity is required only after process launch.",
...
export async function classifyWithJev(input) {
  ...
  for (const source of run.sources.filter((/** @type {any} */ entry) => proposal.sourceIds.includes(entry.id))) {
    const excerpt = await readBoundedText(join(run.root, source.path), JEV_MAX_SOURCE_BYTES);
  ...
  if (obligations.some((id) => ["changed_scope", "repo_patterns", "docs_consistency"].includes(id))) {
    ...
    const collect = async (root, paths, provenance) => {
      for (const file of await snapshotPaths(root, paths)) {
        ...
        seen.add(key);
        const { stdout: diff } = await execFileAsync("git", ["diff", "--no-ext-diff", "--no-textconv", "--no-color", "--unified=4", "HEAD", "--", file.path], { cwd: root, timeout: 5000, maxBuffer: JEV_MAX_CHANGED_CONTEXT_BYTES });
        const metadataOnly = !diff && file.state === "file" && (file.size ?? 0) > JEV_MAX_SOURCE_EXCERPT_BYTES;
        const contents = diff || file.state === "missing" || metadataOnly ? null : await readBoundedText(join(root, file.path), JEV_MAX_CHANGED_CONTEXT_BYTES);
        ...
        if (contentBytes > JEV_MAX_CHANGED_CONTEXT_BYTES) throw new GovernanceError("required changed-file context exceeds the classifier byte cap");
  ...
  let body;
  try { body = buildBody(run, proposal, obligations, candidates, sources, criteria, { ...completed, changeEvidence }, applicability); }
  catch { return { ok: false, reason: "required classifier context cannot be packed without loss or ambiguity", code: "context_unavailable", transportAttempted: false }; }
