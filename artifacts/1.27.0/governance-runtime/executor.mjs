...
export async function launchGovernedProcess({ home, runDirectory, sessionId, launch, inputPath, signal }) {
  ...
  try {
    ...
    let observed = { provider: "unknown", model: "unknown", reasoning: /** @type {string|null} */ ("unknown"), output: "Provider identity could not be established from actual process metadata.", sessionId: /** @type {string|undefined} */ (undefined) };
    try {
      ...
      changed = flash ? repositoryDelta(baseline ?? [], await snapshotRepository(candidateRoot)) : [];
      readScopeChanged = (codex || check) && (before !== stableHash(await snapshotPaths(root, proposal.readSet)) || schemaPath !== null && await readFile(schemaPath, "utf8").catch(() => null) !== schemaText);
