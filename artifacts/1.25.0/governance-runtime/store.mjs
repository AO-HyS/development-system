...
/** @param {string} home @param {string} sessionId @returns {Promise<string | null>} */
export async function registryRunDirectory(home, sessionId) {
  const registry = await readRegistry(home);
  ...
  const entry = matches.find((/** @type {any} */ candidate) => candidate.finished !== true) ?? matches.at(-1);
