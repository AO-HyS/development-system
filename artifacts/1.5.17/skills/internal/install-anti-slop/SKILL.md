...
This Development System adapter is the single contained entrypoint for the
vendored `dmmulroy/anti-slop` installer (upstream commit
`e8c4880471b23ab7f216fba7b27d173a6ef07d4c`, MIT). The pristine upstream
installer source is preserved unchanged under
`artifacts/1.5.17/skills/upstream/install-anti-slop` for provenance only;
always install through this adapter's contained `scripts/install.mjs`, never
with raw upstream bytes.

## Containment contract

Run installs only with:

```bash
node <skill-directory>/scripts/install.mjs [relative-destination] [--force]
```

The installer refuses before any copy:

- absolute destinations;
- empty, dot (`.`), or parent (`..`) path segments, including nested traversal such as `tools/../../outside`;
- backslash destinations (ambiguous across platforms);
- any symbolic link ancestor between the working directory and the destination, an existing symlink at the destination, any symbolic link already nested inside an existing destination (even with `--force`), and a working directory reached through a symbolic link.

`--force` keeps its upstream meaning: it may overwrite an existing regular destination after review, but it never authorizes writing through a symlink or outside the working directory.
...
2. Copy the bundled plugin from this skill. Run from the target repository:
   ...
   This creates `tools/oxlint/anti-slop/` through the contained installer. Pass another relative destination as the first argument when the repository has an established tooling layout. The script refuses to replace an existing destination; only use `--force` after backing up and reviewing existing files.
