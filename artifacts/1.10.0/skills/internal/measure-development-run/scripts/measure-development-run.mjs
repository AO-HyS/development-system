    ...
    for (const entry of await readdir(current, { withFileTypes: true })) {
      ...
      try {
        for await (const line of lines) {
          ...
          if (header.type === "session_meta" && parent === parentId &&
              uuidPattern.test(payload.id ?? "") && timestampMs(header.timestamp) <= cutoffMs) {
            children.push({ threadId: payload.id, timestamp: header.timestamp });
          }
          break;
        }
      } catch { /* An incomplete header is unavailable, not evidence of execution. */ }
...
function collectUuids(value, seen, found) {
  if (typeof value === "string") {
    const matches = value.match(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/gi);
    if (matches) {
  ...
  if (isRecord(value)) {
    for (const [key, entry] of Object.entries(value)) {
      if (/agent|thread|child|task|spawn/i.test(key)) collectUuids(entry, seen, found);
    }
    // Fall back to nested scan only through likely containers to avoid false positives.
    for (const [key, entry] of Object.entries(value)) {
      if (!/agent|thread|child|task|spawn|output|result|data/i.test(key)) continue;
      if (typeof entry === "object" && entry !== null) collectUuids(entry, seen, found);
    }
  }
}
...
function extractSpawnChildIds(output) {
  ...
  if (Array.isArray(output)) {
    for (const item of output) {
      if (isRecord(item) && typeof item.text === "string") collectUuids(item.text, seen, found);
...
function buildBoundedIntervals(taskEvents, cutoffMs) {
  const starts = taskEvents
    .filter((event) => isTaskStartType(event.kind) || event.kind === "task_started")
    .map((event) => ({ time: timestampMs(event.timestamp), turnId: event.turnId ?? null, event }))
    .filter((entry) => entry.time <= cutoffMs)
    .sort((left, right) => left.time - right.time);
  const ends = taskEvents
    .filter((event) => isTaskEndType(event.kind) || event.kind === "task_complete")
    .map((event) => ({ time: timestampMs(event.timestamp), turnId: event.turnId ?? null, event }))
    .filter((entry) => entry.time <= cutoffMs)
    .sort((left, right) => left.time - right.time);
  ...
  for (let index = 0; index < starts.length; index += 1) {
    const start = /** @type {{time: number, turnId: string | null, event: any}} */ (starts[index]);
    const nextStartTime = index + 1 < starts.length
    ...
    if (start.turnId) {
      ...
      if (candidate && candidate.time >= start.time && candidate.time <= endTime) matchedEnd = candidate;
    }
    if (!matchedEnd) {
      const globalEnd = ends.find((end) =>
        end.time >= start.time
        && end.time <= endTime
    ...
    if (matchedEnd && typeof matchedEnd.event.durationMs === "number" && Number.isFinite(matchedEnd.event.durationMs)) {
      const reportedStart = Math.max(start.time, matchedEnd.time - Math.max(0, matchedEnd.event.durationMs));
      const interval = {
        startedAt: new Date(Math.min(reportedStart, matchedEnd.time)).toISOString(),
  ...
  for await (const line of lines) {
    ...
    if (record.type === "event_msg" && isRecord(record.payload)) {
      ...
      if (isTaskStartType(type) || isTaskEndType(type)) {
        ...
        if ((type === "error" || type === "interruption") && !asString(record.payload.turn_id)) continue;
        taskEvents.push({
          timestamp,
          kind: type ?? "unknown",
    ...
    if (payloadType === "custom_tool_call_output" || payloadType === "function_call_output") {
      ...
      if (call && call.name === "spawn_agent" && !status.failed) {
        for (const childId of extractSpawnChildIds(record.payload.output)) {
          if (childId !== session?.id) spawnChildren.push({ timestamp, threadId: childId, callId });
        }
      }
    }
  }
...
async function childAgentDetails(sessionsRoot, childThreadIds, cutoffMs, spawnTimes = new Map()) {
  ...
  for (const threadId of childThreadIds) {
    try {
      ...
      const context = metadata.contexts
        .filter((item) => inWindow(item.timestamp) && timestampMs(item.timestamp) <= cutoffMs)
        .sort((left, right) => timestampMs(left.timestamp) - timestampMs(right.timestamp))
        .at(-1);
      const taskEvents = metadata.taskEvents.filter((event) => inWindow(event.timestamp));
      ...
      const ownedIntervals = lowerBoundMs === undefined ? bounded.intervals : bounded.intervals.filter(
        (interval) => timestampMs(interval.endedAt) > lowerBoundMs,
      ).map((interval) => ({
        startedAt: new Date(Math.max(timestampMs(interval.startedAt), /** @type {number} */ (lowerBoundMs))).toISOString(),
        endedAt: interval.endedAt,
      })).filter((interval) => timestampMs(interval.endedAt) > timestampMs(interval.startedAt));
      const durationMs = ownedIntervals.length > 0 ? unionDurationMs(ownedIntervals) : null;
      const startedAt = ownedIntervals
        .map((interval) => interval.startedAt)
        .sort()
        .at(0) ?? null;
      const hasOpen = bounded.openIntervals.length > 0
        && (lowerBoundMs === undefined || bounded.openIntervals.some(
          (interval) => timestampMs(interval.endedAt) > lowerBoundMs,
        ));
      const endedAt = hasOpen
        ? null
        : ownedIntervals.map((interval) => interval.endedAt).sort().at(-1) ?? null;
      const orderedTokens = metadata.tokenEvents
        .filter((event) => inWindow(event.timestamp))
        .sort((left, right) => timestampMs(left.timestamp) - timestampMs(right.timestamp));
      const previousToken = metadata.tokenEvents
        .filter((event) => lowerBoundMs !== undefined && timestampMs(event.timestamp) < lowerBoundMs)
        .sort((left, right) => timestampMs(left.timestamp) - timestampMs(right.timestamp)).at(-1);
      const baseline = isRecord(previousToken?.usage) ? asNumber(previousToken.usage.total_tokens) : null;
      const values = orderedTokens.map((event) => isRecord(event.usage) ? asNumber(event.usage.total_tokens) : null)
        .filter((value) => typeof value === "number");
      const childTokensTotal = lowerBoundMs === undefined || values.length === 0 ? null
        : (sumCumulativeWithResets(baseline === null ? values : [baseline, ...values]) ?? 0) - (baseline ?? 0);
      ...
      details.push({
        ...
        telemetryStatus: lowerBoundMs === undefined
          ? "observed-unbounded: no evidenced spawn time; inherited history cannot be excluded"
          : "observed",
        executionStatus: hasOpen ? "in-progress" : endedAt ? "completed" : "unavailable",
        startedAt,
        endedAt,
...
export async function collectTelemetry(options) {
  ...
  const completedTurns = taskEvents.filter(
    (event) => (event.kind === "task_complete" || event.kind === "turn_complete"
      || event.kind === "task_completed" || event.kind === "turn_completed") && event.durationMs !== undefined,
  );
  const endedTurnIds = new Set(
    taskEvents.filter((event) => isTaskEndType(event.kind)).map((event) => event.turnId).filter(Boolean),
  );
  const openTurns = taskEvents.filter(
    (event) => isTaskStartType(event.kind) && event.turnId && !endedTurnIds.has(event.turnId),
  );
  const bounded = buildBoundedIntervals(taskEvents, cutoffMs);
  const completedParentTurnMs = unionDurationMs(bounded.completedIntervals);
  const operationalIntervals = bounded.intervals;
  const parentActiveMs = operationalIntervals.length > 0 ? unionDurationMs(operationalIntervals) : 0;
  const inProgressParentTurnMs = bounded.openIntervals.length > 0
  ...
  /** @param {string} field */
  const summedTokenField = (field) => sumCumulativeWithResets(
    orderedTokenEvents
      .map((event) => isRecord(event.usage) ? asNumber(event.usage[field]) : null)
      .filter((value) => typeof value === "number"),
  );
  ...
  const headerChildren = metadata.agentSpawns.length > 0
    ? await discoverChildHeaders(options.sessionsRoot, metadata.session.id, cutoffMs) : [];
  for (const spawn of [...headerChildren, ...(metadata.spawnChildren ?? [])]) {
    ...
    if (!spawnTimes.has(spawn.threadId) || timestampMs(spawn.timestamp) < (spawnTimes.get(spawn.threadId) ?? Infinity)) {
      spawnTimes.set(spawn.threadId, timestampMs(spawn.timestamp));
    }
  }
  const childThreadIds = [...new Set([...agents.childThreadIds, ...spawnTimes.keys()])];
  const children = await childAgentDetails(options.sessionsRoot, childThreadIds, cutoffMs, spawnTimes);
  ...
  return {
    ...
    tokens: orderedTokenEvents.length > 0
      ? {
          ...
          allAgentsTotal: (summedTokenField("total_tokens") ?? 0) + childTokens,
          observedAt: tokenEvent?.timestamp ?? null,
          status: "summed-nonnegative-increments-across-resets; child sessions added separately",
        }
      : {
          input: null,
          cachedInput: null,
    ...
    turns: {
      ...
      abortedOrInterrupted: taskEvents.filter(
        (event) => isTaskEndType(event.kind) && event.kind !== "task_complete"
          && event.kind !== "turn_complete" && event.kind !== "task_completed"
...
export async function persistMeasurement(options) {
  ...
  const unsigned = {
    ...
    capturedAt,
    priorityOrder: ["verified-outcome", "quality-architecture-security", "scope", "speed", "cost"],
    compositeScore: null,
