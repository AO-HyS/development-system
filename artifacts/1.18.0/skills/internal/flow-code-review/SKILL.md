...
description: Review a branch or diff against Standards and Spec, using independent assistance when risk warrants it, and rank actionable findings.
---
...
For ordinary changes, the selected reviewer owns both inspections and the integrated judgment.
When independent reviewers are selected below, keep their packets blind.
...
## 3. Review both axes with proportional isolation

Inside a Git repository, load `coding-orchestration`. The selected model reviews both axes directly for ordinary changes. Use one independent capable review with separate Standards and Spec notes when security, migrations, cross-cutting changes, or a user request justify independent review. Use two blind reviewers only when their independent investigations materially reduce risk. Respect the selected host, provider restrictions and actual capabilities; the roster is optional guidance.
...
The findings, Standards/Spec verdict and remaining evidence gaps complete an
ordinary review. Provide a standalone review document when requested or needed
for the agreed durable handoff. Use an available renderer and its format guide
only for that document; a useful Markdown report is sufficient when no renderer
is available. A visualization is optional when it clarifies a complex change.
Continue corrections and delivery when the user already authorized them.
