...
array from `model-route` for the parent-selected model into this execution
packet. Store it privately; prompts may contain sensitive project context. The
OpenCode Go command shown below is an example, not a provider fallback; an
SWE-2 packet keeps every descendant in its selected SWE family, including
capable critics and browser roles.
