...
async function validateTool(event, run, proposal, home, descriptor) {
  ...
  if (typeof root !== "string" || await realpath(event.cwd) !== await realpath(root)) invalid("Tool cwd does not match the run root.", "binding");
  ...
  if (descriptor.capability === "patch") {
    ...
    for (const path of patchPaths(patch)) if (!covered(await validateWritablePath(root, path, home), writeSet)) invalid("Patch path is outside the prepared write scope.", "scope");
    return;
  }
...
export async function handleHook(input, { home = homedir() } = {}) {
  ...
  try {
    ...
    // Read-only governance diagnostics are exempt active or unbound; execute is
    // never in GOVERNANCE_COMMANDS and therefore never exempt here.
