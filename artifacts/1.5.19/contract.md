# Development System Contract 1.5.19

Version 1.5.19 installs one human-editable agent roster as the source of truth
for roles, timing, ordered provider fallbacks, reasoning, availability gates,
and Codex priority/fast service selection. The orchestration planner and the
installed `coding-orchestration` skill both consume that roster. Published
version snapshots remain immutable; invalid roster edits fail closed.

Version 1.5.19 preserves the immutable 1.5.18 contract and catalog 0.26.0,
the fast-model-first route, the independent Fable/Sol reviewer route, and
every executable anti-slop lane contract. Catalog 0.26.0 replaces only two
overly brittle response signatures with equivalent canonical terms observed
in substantive answers. Probe prompts still contain none of their own
behavior signatures, and a real read of the exact installed `SKILL.md`,
catalog discovery, a successful harness invocation, and a substantive answer
to the skill-specific question remain mandatory.
...
Published 1.5.17 and 1.5.16 bytes plus catalogs 0.24.0 and 0.23.0 remain
untouched rollback targets.

## Skill catalog 0.26.0 probe reliability patch

Catalog `0.26.0` reuses the exact skill payloads and provenance from 0.24.0.
It changes only the canonical response signatures for `behavioral-evidence`
...
Repository initialization and normalization record Development System 1.5.19,
skill catalog `0.26.0`, and the same executable anti-slop lane contract
...
contract surface, never a product-specific copy. 1.5.17 remains the rollback
