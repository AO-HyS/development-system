...
Before selecting any agent or model, read
`$HOME/.codex/development-system/agent-roster.json` completely. It is the
installed source of truth for roles, timing, ordered candidates, reasoning,
runtime-availability requirements, and the Codex priority/fast service tier.
Follow the matching `routeSlot`; do not substitute model names from this skill.
...
- The ordinary implementation route and all mechanical, non-deliberative work
  ...
  collection, focused test execution) follow the installed `fast-execution`
  route exactly.
