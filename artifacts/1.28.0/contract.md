# Development System Contract 1.26.0
...
a new agent or conversation for each phase. Tiny bounded work can use direct execution and parent review. Governed features require Astra planning and a different fresh Astra plan review before writing. UX, DX and AX guide the smallest useful
...
command validates files and hashes before recording its recap step. Ordinary ungoverned conversation closure relies on the loaded skill. Activated governed sessions also use a bounded Stop hook on supported hosts; it cannot certify unsupported surfaces. Working Backwards retains its workflow
...
## Governed development profile

New sessions default to Sol High; keep the already selected parent. Definition
uses Astra XHigh for the spec and linked tickets. Implementation can start in a
different thread with only that spec: resolve every linked authorized ticket.
Sol localizes the task; fast read-only researchers collect source evidence;
Astra XHigh authors the implementation plan; a distinct fresh-context Astra
XHigh reviews it before writers start. The reviewer receives requirements,
rules, plan and source evidence, without the author's conversation.

Jev checks each lifecycle boundary: intake and coverage, research and tools,
plan and independent review, every dispatch, order and parallel ownership,
returned changes and repository rules, corrections, deep review and closure.
Jev classifies supplied evidence; the LLM decides the technical response;
deterministic runtime gates enforce current receipts and execution constraints.
Missing, stale, unavailable or insufficient judgments do not grant a pass.

Use the installed coding-orchestration/references/jev-governance.md recipe.
Fast research defaults to the available native Luna High profile; it is a
replaceable capability, not a compulsory model family. Flash or other permitted
profiles require observed availability and an explicit route. Flash High writes
bounded exact packets. Astra XHigh handles planning and independent reviews.
No silent fallback. Native configuration requests a model; host/provider
metadata establishes actual identity and capabilities.

Run eligible disjoint packets concurrently within measured host/provider limits.
Serialize shared paths, dependent work and unisolated provider state. Receive
completion events; terminate the former writer before transferring ownership.
Nonvisual work needs the same requirement, review and behavior evidence chain;
browser and visual checks apply only where the behavior needs them.

Use only certified host surfaces for enforced execution. Keep unsupported
surfaces explicit and blocked when required by the task; do not describe
installed files or advisory receipts as operational enforcement. User stop and
interruption remain authoritative. Continue authorized corrections to actual
acceptance and the authorized endpoint.
...
## Explicit Astra writing (1.27.3)

A deliberately selected Codex/OpenAI gpt-6-astra xhigh route may research or write. Existing profiles remain available; no provider fallback is introduced. A Codex writer runs in a third isolated candidate workspace with a pinned confined permission policy. Every writer receives the same baseline, actual delta, owned-path reconciliation and separate integration checks. Command validation precedes process launch and remains enforced for process events. Requested flags are not observed model identity. Existing permits, Jev boundaries, leases, independent review and acceptance rules remain required. Native writable descendants remain unsupported. Catalog 0.46.1 is retained; its default writer recommendation does not prevent an explicit Astra packet. Maintenance follows ADR 0048 and does not claim that a defective runtime certifies its own repair.
