...
import { createRun, classifyBoundary, prepareAction, advancePhase, confirmCoordinatorEffort, closeRun, getRun, resolveRunDirectory, preflightRun, recoverUnstartedRun, recoverHostAttempt, safeAttemptDiagnostic } from "./core.mjs";
...
export const GOVERNANCE_COMMANDS = Object.freeze(["begin", "preflight", "classify", "prepare", "status", "advance", "confirm-effort", "close", "recover", "recover-host-attempt", ...REFERENCE_COMMANDS]);
  ...
  try {
    ...
    if (command === "status") {
      ...
      if (command === "recover") {
        ...
        if (command === "confirm-effort" && !options["--input-json"]) invalid("Effort confirmation requires exact inline input.", "handoff", "input");
        ...
        else if (command === "advance") result = await advancePhase({ runDirectory, transition: input, argv: process.argv.slice(1) });
        else if (command === "confirm-effort") result = await confirmCoordinatorEffort({ runDirectory, confirmation: input, argv: process.argv.slice(1) });
    ...
    if (["begin", "advance", "confirm-effort", "close", "status", "recover", "recover-host-attempt"].includes(command) && result?.runId) {
      result = { runId: result.runId, root: result.root, hostRoot: result.hostRoot ?? result.root, rootSessionId: result.rootSessionId,
        coordinator: { actorId: result.coordinator.actorId, role: result.coordinator.role, provider: result.coordinator.provider, model: result.coordinator.model, reasoning: result.coordinator.reasoning, sessionId: result.coordinator.sessionId },
        coordinatorIdentityEpoch: result.coordinatorIdentityEpoch ?? 0,
        pendingCoordinatorEffortTransition: result.pendingCoordinatorEffortTransition ? { descriptorHash: result.pendingCoordinatorEffortTransition.descriptorHash, to: result.pendingCoordinatorEffortTransition.transition.to, boundaryId: result.pendingCoordinatorEffortTransition.boundaryId } : null,
        ...
        plans: result.plans.map((/** @type {any} */ plan) => ({ id: plan.id, actorId: plan.actorId, attemptId: plan.attemptId, boundaryId: plan.boundaryId, candidateHash: plan.candidateHash, source: plan.source, summary: plan.summary, criterionIds: plan.criterionIds, invalidated: plan.invalidated === true,
          packets: plan.packets.map((/** @type {any} */ packet) => ({ id: packet.id, readSet: packet.readSet, writeSet: packet.writeSet, dependsOn: packet.dependsOn })) })),
