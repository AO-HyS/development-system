...
export function validateProposal(input) {
  ...
  rejectUnknownKeys(input, ["id", "phase", "action", "actorId", "attemptId", "objective", "requirementIds", "sourceIds", "readSet", "writeSet", "dependsOn", "route", "toolName", "toolInput", "evidenceRefs", "observations", "coordinatorEffortTransition"], "proposal");
  ...
  return {
    ...
    ...(input.coordinatorEffortTransition === undefined ? {} : { coordinatorEffortTransition: validateCoordinatorEffortTransition(input.coordinatorEffortTransition) }),
  };
}
...
export function validateCoordinatorEffortTransition(input) {
  ...
  rejectUnknownKeys(input, ["sessionId", "provider", "model", "oldEffort", "newEffort", "to"], "coordinatorEffortTransition");
  for (const key of ["sessionId", "provider", "model", "oldEffort", "newEffort", "to"]) if (!isNonEmptyString(input[key])) throw new GovernanceError(`coordinatorEffortTransition.${key} is required`, "invalid");
  assertSafeId(input.sessionId, "coordinatorEffortTransition.sessionId");
  if (!['low', 'medium', 'high', 'xhigh', 'max'].includes(input.newEffort)) throw new GovernanceError("unsupported coordinator effort", "invalid");
  if (input.oldEffort === input.newEffort) throw new GovernanceError("coordinator effort must change", "invalid");
  return { sessionId: input.sessionId, provider: input.provider, model: input.model, oldEffort: input.oldEffort, newEffort: input.newEffort, to: input.to };
}
...
export function validateTransition(input) {
  ...
  rejectUnknownKeys(input, ["to", "reason", "boundaryId", "coordinatorEffortTransition"], "transition");
...
export function validateHostEvent(input) {
  ...
  if (input.kind === "process-start" || input.kind === "process-exit") {
    rejectUnknownKeys(input, ["kind", "attemptId", "attempt_id", "processId", "process_id", "provider", "model", "reasoning", "exitCode", "exit_code", "exitSignal", "reconciliationFailed", "terminated", "cancelled", "output", "changedPaths", "changed_paths", "candidateRoot", "processSessionId", "command", "serviceTierObservation"], "event");
    ...
    if (input.kind === "process-start" && input.serviceTierObservation !== undefined) throw new GovernanceError("service tier observation requires process exit", "identity");
    if (input.kind === "process-exit") {
      if (input.serviceTierObservation !== undefined && (!isRecord(input.serviceTierObservation) || Object.keys(input.serviceTierObservation).some((key) => !["requested", "hostObserved", "providerObserved"].includes(key)) || !["priority", "default", "unspecified"].includes(input.serviceTierObservation.requested) || !["priority", "default", "unknown"].includes(input.serviceTierObservation.hostObserved) || !["priority", "default", "unknown"].includes(input.serviceTierObservation.providerObserved))) throw new GovernanceError("invalid service tier observation", "identity");
    ...
    return {
      ...
      ...(input.serviceTierObservation === undefined ? {} : { serviceTierObservation: structuredClone(input.serviceTierObservation) }),
    };
  }
  const sessionId = pick("session_id", "sessionId");
  const attemptId = pick("attempt_id", "attemptId");
