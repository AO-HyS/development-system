...
import { authorizeAction, closeRun, getRun, recordHostEvent, registerHostSession, recordPassiveToolObservation, resolveRunDirectory, stableHash, recordEffortControlInvocation } from "./core.mjs";
...
export async function observeHostIdentity(event) {
  ...
  try {
    ...
    let sessionSeen = false, child = false, turnOrdinal = 0, observedTurnOrdinal = null;
    ...
    try {
      for await (const line of lines) {
        ...
        if (record.type === "turn_context") {
          turnOrdinal += 1;
          if (metadata.turn_id === event.turn_id && metadata.model === event.model) { turn = metadata; observedTurnOrdinal = turnOrdinal; }
        }
      }
    ...
    return { kind: "session", sessionId: event.session_id, cwd: await realpath(event.cwd), model: event.model,
      transcriptPath: await realpath(event.transcript_path), reasoning: typeof effort === "string" ? effort : null, turnOrdinal: observedTurnOrdinal };
  } catch { return null; }
...
async function stopResponse(runDirectory, run) {
  ...
  if (pending && pending.descriptorHash === stableHash(pending.descriptor)
    && run.phase === run.boundaries.find((/** @type {any} */ boundary) => boundary.id === pending.boundaryId && boundary.judgmentId === pending.judgmentId)?.phase
    && !run.attempts.some((/** @type {any} */ attempt) => ["running", "awaiting-post", "cancelling", "recovery-required"].includes(attempt.status))
    && Object.keys(run.leases ?? {}).length === 0) return {};
...
export async function handleHook(input, { home = homedir() } = {}) {
  ...
  try {
    ...
    if ((event.hook_event_name === "PreToolUse" || event.hook_event_name === "PostToolUse")
      && event.tool_name === "Bash" && await isGovernanceControlCommand(event.tool_input?.command, { home })) {
      const words = tokenizeExactCommand(event.tool_input.command);
      if (!words) return denyTool("Effort control command is malformed.");
      const parsed = parseGovernanceArguments(words.slice(2));
      if (!["advance", "confirm-effort", "classify", "prepare", "execute"].includes(parsed.command)) return {};
      ...
      const isEffortAdvance = parsed.command === "advance" && Boolean(parsed.options["--input-json"] && JSON.parse(parsed.options["--input-json"]).coordinatorEffortTransition);
      if (event.hook_event_name === "PreToolUse" && run?.pendingCoordinatorEffortTransition && parsed.command !== "confirm-effort" && !["status", "close"].includes(parsed.command)) return denyTool("Coordinator effort confirmation is pending.");
      if (event.hook_event_name === "PreToolUse" && (isEffortAdvance || parsed.command === "confirm-effort")) {
        const observation = await observeHostIdentity(event);
        if (!observation || !runDirectory || !parsed.options["--input-json"] || !event.turn_id || !event.tool_use_id) return denyTool("Effort control requires an exact observed host invocation and inline input.");
        await recordEffortControlInvocation({ runDirectory, command: parsed.command, input: JSON.parse(parsed.options["--input-json"]), fullCommand: event.tool_input.command, argv: words.slice(1), event, observation });
      }
      return {};
    }
    const runDirectory = await resolveRunDirectory(home, event.session_id, { includeFinished: false });
