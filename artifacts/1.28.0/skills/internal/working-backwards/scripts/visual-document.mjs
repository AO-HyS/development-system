...
export function renderVisualDocument(model, helpers) {
  ...
  const evidenceHtml = evidence ? evidence.replace(/(<h2 id="delivery-evidence"[^>]*>)/u, (tag) => `${tag}${numeral()}`) : "";
  const content = sections.map(section => {
    ...
    return `<section class="brief-section"${section.heading ? ` aria-labelledby="${esc(section.heading.anchorId)}"` : ""}>${section.heading ? `<div class="section-heading">${heading}${intro ? `<div class="section-intro">${render(intro)}</div>` : ""}</div>` : ""}<div class="section-content">${section.blocks.filter(block => block !== intro).map(render).join("\n")}</div></section>`;
  }).join("\n");
  ...
  const findings = doc.findings?.length ? `<section class="findings" aria-labelledby="findings-title"><h2 id="findings-title" class="notebook-label">${es ? "Hallazgos" : "Findings"}</h2><ol>${doc.findings.map((/** @type {Record<string, any>} */ finding, index) => `<li class="finding" data-q="finding-${index + 1}"><span class="finding-num" aria-hidden="true">${index + 1}</span><p><strong>${esc(finding.title)}.</strong> ${helpers.inlineMarkdown(string(finding.detail))}</p>${finding.status ? `<span class="finding-status status-${esc(finding.status)}">${statusMark[/** @type {"verified"} */ (finding.status)]}${statusLabel[finding.status]}</span>` : "<span></span>"}</li>`).join("")}</ol></section>` : "";
