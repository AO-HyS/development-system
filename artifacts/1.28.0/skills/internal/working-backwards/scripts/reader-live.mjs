...
export async function startReaderLive(input) {
  ...
  async function handleQuestions(request, response, rest) {
    ...
    const directory = resolve(workspaceDir, ".questions", ...rest.slice(0, -1), name.replace(/\.questions\.json$/u, ""));
    if (!(await assertReadableRegularFile(workspaceDir, reader)) || !(await assertSafeDirectory(workspaceDir, directory))) { response.writeHead(404).end(); return; }
