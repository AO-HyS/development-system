...
Read [Jev governed execution](references/jev-governance.md) once for non-trivial work. Keep the
...
verification, findings and the terminal state. The governance runtime constrains this parent; it does not replace it with another coordinator. Execute the repository's
