# Model routing operator contract (1.11.0)
...
Medium, then Astra High. Ordinary review stays with Astra. Clear orchestration
requests Low. Escalation raises Low to Medium and other Astra routes to High
when observed risk justifies it. Effort is a
...
The requested effort is policy, not a change to an already-running T3 turn.
Do not claim that API async tools or configuration_update are enabled in Codex/T3
because the model supports them. Those require host integration and separate
runtime evidence. Use available concurrent tools today; increase effort for
observed difficult judgment rather than raising every lane globally.

On T3, execute authorized CLI workers through `run-worker`; see the installed
coding-orchestration/worker-reference.md. Owner polling is deterministic, with
private logs and a terminal receipt. No fixed productive-task timeout is imposed.

## Evidence and visual completion

Astra retains live Computer Use and independent visual critique. Complete
Impeccable and resolve the independent critic's findings before recording the
final evidence package. Diagnostic screenshots and probes may precede critique.
The `evidence-preparation` route selects Codex Luna High only for bounded script
writing or existing-media processing. It does not transfer browser control or
acceptance. The general implementation fallback order is unchanged.
Use host-native tool batches for stable steps, one driver per session, and
compact completion receipts. Do not install another interpreter or daemon when
the host already supports composition. These are routing rules, not measured
claims of lower subscription usage; compare completed outcomes in real runs.
