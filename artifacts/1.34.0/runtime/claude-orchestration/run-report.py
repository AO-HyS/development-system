...
#   python3 run-report.py [<session-id>] [--project <project-dir-name>]
# Without a session id it uses the newest session in the ledger; without --project it
# reads the project directory derived from the current directory. Costs are estimates
...
for a in argv:
    if a == '--project': project = next(argv, None)
    elif a.startswith('--project='): project = a.split('=', 1)[1]
    else: args.append(a)

rows = [json.loads(l) for l in open(LEDGER)]
session = args[0] if args else rows[-1]['session']
...
def usage(file):
    # A streamed message is written once per content block; only its last row carries the
    # final usage (the first chunk undercounts output), so keep the last usage per message.id.
    last, tot, model, images = {}, collections.Counter(), None, 0
    for line in open(file):
        ...
        if d.get('type') != 'assistant' or not m.get('usage'): continue
        last[m.get('id') or f'line-{len(last)}'] = m['usage']
...
# One project directory only, never a glob across projects: --project <dir-name> (or the
# legacy second positional argument), else the directory Claude Code derives from cwd.
project = args[1] if len(args) > 1 else project or re.sub(r'[^A-Za-z0-9]', '-', os.getcwd())
main = [f for f in [os.path.join(PROJECTS, project, f'{session}.jsonl')] if os.path.exists(f)]
if not main: print(f'No transcript {session}.jsonl in {os.path.join(PROJECTS, project)}; pass --project <dir-name>.', file=sys.stderr)
subs = glob.glob(os.path.join(PROJECTS, project, session, 'subagents', 'agent-*.jsonl'))
