...
- Use `fast_implementer` on Spark Low for exact bounded implementation.
- If Spark is unavailable or fails the same bounded packet twice, use the
  fastest currently validated implementation route; do not wait for Luna or
  escalate to Sol merely because it is familiar.
