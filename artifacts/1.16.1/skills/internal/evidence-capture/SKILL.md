...
description: Capture a real UI walkthrough, prepare screenshots or video evidence, or package existing media for acceptance. Use when a task requires recorded product behavior.
---

Astra owns live Computer Use, chooses acceptance and judges the result.
Deterministic host tools capture and package it under Astra supervision. Use the `evidence-preparation` roster route (Luna) only when a
bounded script or media correction needs a writer; a running recording needs
no model agent. Keep one driver per browser session.

1. For final delivery, require completed Impeccable work and independent Astra
   visual critique with findings resolved on this candidate (`design-quality`).
...
The executor reports complete/incomplete, never product PASS/FAIL. Astra combines
it with before/after probes and visual review. Missing recording support remains
a gap; screenshots or generated video cannot impersonate a walkthrough.
