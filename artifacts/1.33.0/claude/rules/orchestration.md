...
| Work | Sonnet | Opus low | Opus medium | Opus high | Fable 5.1 |
| --- | --- | --- | --- | --- | --- |
...
files); visual-reviewer (Opus high) for mock vs live page, fresh each time; browser-qa
(Opus low) for browser behavior checks through scripts. There is no per-subagent
...
surfaces and why Opus high is not enough. Astra/Codex are not available right now.
