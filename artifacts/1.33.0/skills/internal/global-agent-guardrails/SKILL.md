...
- Known false positives, by design: interpreter code naming a shell next to a process API (`shutil.which('bash')` with `subprocess` in the same program), an interpreter with only preload options and no program (`node -r ts-node/register`), backtick prose with a destructive command inside a runner argument, glob or brace targets that could match a protected path, read-only editor sessions (`vim -R`, `view`) on a protected or test path, editor script text outside substitutions and typed lines that reads like a write, open or shell command (`:echo "e tests/a.test.ts"`), and conservative vimscript blocks (`:make`, `:grep`, `:source`, `:normal` keys containing `!` or `@`). Known gaps: editor settings such as `.vscode/settings.json` are not protected; editor script files (`-s`, `-S`, `-u`, `ed < file`), a `SHELL` prefix for the editor's `:!` and vimscript beyond the forms above are not read. The engine denies input over 64 KB and evaluations that exceed its own 3 s budget; the hook command denies when the engine exits abnormally, and the harness enforces the 5 s hook timeout, whose outcome on expiry is the harness's own behavior and is not verified here.
