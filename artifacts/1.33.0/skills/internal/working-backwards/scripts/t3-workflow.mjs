...
function validateArchitectureConvergenceTechnicalGrill(content) {
  if (frontmatterValue(content, "working_backwards_program") !== "architecture-convergence") return;
  const rows = new Map();
  const duplicates = new Set();
  for (const line of content.split(/\r?\n/u)) {
    const cells = line.split("|").slice(1, -1).map((cell) => cell.trim());
    if (cells.length < 4 || !ARCHITECTURE_CONVERGENCE_DIMENSIONS.includes(cells[0])) continue;
    if (rows.has(cells[0])) duplicates.add(cells[0]);
    rows.set(cells[0], cells.slice(1, 4));
  }
  const placeholder = /^(?:|[-—]|todo|tbd|pendiente|por definir|unknown|desconocido)$/iu;
  const disposition = /\b(?:keep|change|remove|unproven|conservar|cambiar|retirar|eliminar|no demostrado)\b/iu;
  const missing = ARCHITECTURE_CONVERGENCE_DIMENSIONS.filter((dimension) => {
    const cells = rows.get(dimension);
    return !cells || cells.some((cell) => placeholder.test(cell)) || !disposition.test(cells[1]);
  });
  if (missing.length > 0 || duplicates.size > 0) {
    const details = [
      missing.length > 0 ? `missing dimensions: ${missing.join(", ")}` : null,
      duplicates.size > 0 ? `duplicate dimensions: ${[...duplicates].join(", ")}` : null,
    ].filter(Boolean).join("; ");
    throw new Error(`Architecture convergence Technical Grill is incomplete; ${details}`);
  }
}
...
export function renderT3Reader(status, options = {}) {
  ...
  const displayArtifact = options.displayArtifact !== undefined ? options.displayArtifact : status.currentPhase.role === "t3-handoff"
    ? null
  ...
  const displayIndex = Math.max(0, WORKING_BACKWARDS_PHASES.findIndex((phase) => phase.role === displayPhase.role));
  ...
  const artifactMarkdown = status.currentPhase.role === "t3-handoff" && options.displayArtifact === undefined
    ? `# T3 Handoff privado
...
Este documento todavía no se ha creado.`;
  ...
  const artifacts = WORKING_BACKWARDS_PHASES.map((phase) => {
    ...
    const historyPath = artifact ? resolve(status.workspaceDir, "reader-history", artifact.fileName.replace(/\.md$/iu, ".html")) : null;
    return {
      id: phase.role,
      label: phase.label,
  ...
  const model = buildTechnicalReaderModel({
    ...
    document: {
      ...
      sourceHref: displayArtifact ? localRelativeHref(readerDir, resolve(status.workspaceDir, displayArtifact.fileName)) : null,
      markdown,
    },
    artifacts,
  });
...
export async function writeT3Reader(input) {
  ...
  if (!(await existsWithoutSymlink(historyDir))) await mkdir(historyDir, { mode: 0o700 });
  ...
  for (const entry of historyEntries) {
    ...
    if (ownership.collision) throw technicalReaderDestinationCollision(ownership.collision);
  }
  ...
  for (const entry of historyEntries) {
    ...
    await writeFile(historyTemporary, renderT3Reader(status, { displayArtifact: entry.artifact, readerDir: historyDir }), { encoding: "utf8", mode: 0o600 });
  ...
  return { ...status, ...library, readerWritten: true, readerHistoryPaths, ...retirement, localWrites: [status.readerPath, ...readerHistoryPaths, library.readerLibraryCatalogPath, library.readerLibraryPath] };
}
...
export async function recordT3Turn(input) {
  ...
  if (before.currentPhase.role === "technical-grill") validateArchitectureConvergenceTechnicalGrill(before.currentArtifact.content);
