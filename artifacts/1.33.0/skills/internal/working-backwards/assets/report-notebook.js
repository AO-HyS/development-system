(() => {
  ...
  const t = es ? {
    ...
    send: 'Enviar al modelo', sentAll: 'Enviadas al modelo', panel: 'Preguntas para el modelo', close: 'Cerrar', askDocument: 'Preguntar sobre el documento',
    ...
    sectionPlaceholder: (name) => `¿Qué quieres preguntar sobre «${name}»?`, sectionLabel: (name) => `Tu pregunta sobre la sección «${name}»`,
    ...
    send: 'Send to the model', sentAll: 'Sent to the model', panel: 'Questions for the model', close: 'Close', askDocument: 'Ask about the document',
    ...
    sectionPlaceholder: (name) => `What do you want to ask about “${name}”?`, sectionLabel: (name) => `Your question about the “${name}” section`,
  ...
  const excerptOf = (block) => {
    if (block.dataset.sectionTitle) {
      ...
      return [block.dataset.sectionTitle, intro ? intro.innerText : ''].join(' — ').replace(/\s+/g, ' ').replace(/ — $/, '').trim().slice(0, 240);
    }
  ...
  function openComposer(block, question) {
    ...
    input.placeholder = sectionName ? t.sectionPlaceholder(sectionName) : t.placeholder;
    composer.querySelector('label').textContent = sectionName ? t.sectionLabel(sectionName) : t.label;
    layout();
  ...
  function closeComposer(restore) {
    ...
    askOrigin = null;
    clearSelection();
    if (restore && origin && origin.isConnected) origin.focus({ preventScroll: true });
    else if (restore && block && block.hasAttribute('tabindex')) block.focus({ preventScroll: true });
  }
  ...
  document.addEventListener('click', (event) => {
    const button = event.target instanceof Element ? event.target.closest('[data-section-ask]') : null;
    if (!button) return;
    const block = button.closest('[data-q]') || (button.dataset.sectionAsk === 'document' ? blockFor('summary') || blockFor('verdict') || document.querySelector('.brief [data-q]') : null);
    if (!block) return;
    if (!composer.hidden && input.value.trim() && active !== block) { input.focus(); return; }
