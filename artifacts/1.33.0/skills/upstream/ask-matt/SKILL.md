...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue with real verification (computer use, the browser, the repository's verification CLI; no automated tests, and tests found are deleted), then closes out by running **`/code-review`**, a two-axis review (Standards + Spec) of the diff, before committing. Reach for **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
...
- **Something's broken** → **`/diagnosing-bugs`**. For the hard ones: the bug that resists a first glance, the intermittent flake, the regression that crept in between two known-good states. It refuses to theorise until it has a **tight feedback loop** (one command that already goes red on *this* bug), then fixes and verifies the fix for real, without automated tests. Its post-mortem hands off to **`/improve-codebase-architecture`** when the real finding is that there's no good seam to lock the bug down.
...
- **`/codebase-design`** is the deep-module vocabulary (module, interface, depth, seam, adapter, leverage, locality) for designing a module's *shape*: a lot of behaviour behind a small interface at a clean seam. `/improve-codebase-architecture` speaks it.
