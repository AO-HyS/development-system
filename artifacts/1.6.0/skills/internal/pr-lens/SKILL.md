...
3. **Validate, and fix**
   ...
   ```bash
   npx @coldtea/pr-lens-cli@0.2.0 validate .pr-lens/graph.json
   ```
...
4. **Render.**
   ...
   ```bash
   npx @coldtea/pr-lens-cli@0.2.0 render .pr-lens/graph.json --theme dark
   ```
   ...
   Keep diagrams local by default and return the generated SVG path. A request
   to explain code does not authorize uploading it. Use a private temporary
   directory when generating outside a requested repository change.
   Run `canvas push` only when the user explicitly authorized publishing this
   diagram to the public PR Lens service. Never expose canvas edit tokens.
   Authorization to open a PR covers its intended description and attachments,
   not a separate public canvas. Verify the target repository and visibility.
...
5. **Attach, when there is a pull request to attach to.** That means the user asked you to open a PR, asked for a diagram on one that exists, or you are opening a PR as part of changes made. Otherwise skip this step.
   ...
   ```bash
   npx @coldtea/pr-lens-cli@0.2.0 comment \
...
Author the graph directly with the current model; `validate` and `render` need
no second LLM account. Do not run provider-backed `analyze` unless the user asks
for that additional inference path. The renderer has no model usage fee;
provider usage and hosting may have independent costs.
...
`references/config.md` has the full format and the recipes. Validate it the same way: `npx @coldtea/pr-lens-cli@0.2.0 validate .github/pr-lens.yml`.
