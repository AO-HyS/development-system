...
export class JevClassificationError extends Error {
  /** @param {string} message @param {any} telemetry */
  constructor(message, telemetry) { super(message); this.name = "JevClassificationError"; this.telemetry = telemetry; }
}

/** @param {any} payload @param {number} started @param {number} requestBytes @param {number} questions */
function failedJevTelemetry(payload, started, requestBytes, questions) {
  ...
  return { model: typeof payload?.model === "string" ? payload.model.slice(0, 128) : null, requestedModel: orchestrationPolicy.model,
    ...
    latencyMs: performance.now() - started, requestBytes, questions };
}

/** @param {{atom: any, run: any, activeAtoms?: any[], contextMode?: "atom" | "compact" | "flow", flowContext?: any, apiKey?: string, fetchImpl?: typeof fetch, endpoint?: string}} input */
export async function classifyAtomWithJev(input) {
  ...
  try {
    ...
    throw new JevClassificationError(error instanceof Error && error.message.startsWith("Jev ") ? error.message : "Jev transport failed",
      failedJevTelemetry(payload, started, requestBytes, Object.keys(questions).length));
