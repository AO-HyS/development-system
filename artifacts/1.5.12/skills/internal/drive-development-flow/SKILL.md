...
description: Route software goals and process discussions through the smallest user-requested lifecycle stage. Definition ceremonies are opt-in; ordinary implementation stays direct while non-trivial work uses bounded fast-model-first orchestration.
---

# Drive Development Flow

Treat a development conversation as a lifecycle. Determine the current stage, load the matching installed skill, and move only as far as the user's request authorizes. Do not turn ordinary feature language into a definition ceremony.

The internal `flow-*` stage overlays live under `~/.agents/skills/` and are consumed by Codex and T3 Code. Do not generate or require a Factory mirror, and do not substitute a similarly named managed upstream skill.
...
| In Codex or T3 Code, the user explicitly invokes `working-backwards` / `work backwards`, or asks for future customer experience, Amazon Working Backwards, PRFAQ/customer story, or one planning document at a time | `working-backwards` |
...
| User explicitly asks to execute several authorized tickets or repositories concurrently | `parallel-work` |
...
If the user explicitly asks for grilling and there is no codebase, use `grill-me` instead of `grill-with-docs`. Never infer Wayfinder, grilling, specification, ticket creation, prototypes, `parallel-work`, a broad suite, or promotion from size, ambiguity, urgency, or the number of tickets. `work-multiple` is only a deprecated migration alias for an explicit Parallel Work request.
...
`product grill -> customer story -> technical grill -> research when needed -> product contract -> technical contract -> implementation map -> private T3 handoff`

The Product Grill is non-technical and proceeds by Topic. It produces the compact Future Customer Story. After that story is approved, a separate Technical Grill derives only the remaining Topics from the story, profile, repository evidence, and risks. Do not repeat settled Topics.
...
The default non-trivial route is Sol High as parent, Luna `code_mapper` or
`docs_researcher` for bounded discovery, Luna `fast_implementer` as the single
writer, Luna `test_runner` for focused verification, and one independent Sol
reviewer. Use specialists only for observed risk, keep depth one, reuse lanes
for corrections, and close every lane with a terminal state. Trivial localized
work remains on the parent.

## Measure the real run without a benchmark

For implementation work, natural evidence may be captured retrospectively from
the session or harness through `measure-development-run`. The
`development-run` operation only normalizes records supplied to it; it does
not auto-wire into a harness or synthesize missing events. When a record is
available, append monotonic phase events for discovery, definition,
implementation, review, correction, checks, CI, provider waits, agent waits,
and human waits as they actually occurred. Record the observed harness and
resolved model; never write `inherit` as evidence. Overlapping intervals are
unioned by the canonical recorder. Missing tokens, cost, quality, CI, or
provider telemetry remain `unproven` and never block delivery. Do not ask the
operator to repeat the work as an A/B benchmark.
