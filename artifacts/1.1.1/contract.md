# Development System contract 1.1.1

Version 1.1.1 retains every 1.1.0 lifecycle, authorization, adapter, benchmark, repository-preparation, delivery, measurement, privacy, installation, and rollback guarantee. It closes shell-spelling bypasses in executable command guardrails and applies local Exa DLP inspection to the complete outbound payload. Published 1.1.0 bytes remain unchanged.
...
- A harness denial cannot be overridden by the agent. A human must review the exact target and recovery plan and act outside or deliberately remove the guard.
...
- Catalog 0.5.1 patches `exa-search` and `global-agent-guardrails` without changing immutable catalog 0.5.0 or any earlier catalog.
