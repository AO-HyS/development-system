...
| `fast_implementer` | Execute exact bounded packets with Luna High for rapid, stable writes. |
...
3. Implementation: default to one writer. Use Luna High `fast_implementer` first for exact bounded packets, Sol Medium `implementer` or `backend_specialist` when judgment is required, or Sol Extra High `ui_designer` for visual craft, with explicit file or package ownership.
...
Use this progression and include the actual route in the final evidence:

1. Luna High for exact bounded implementation packets, focused test execution, and explicit mechanical edits; Luna Extra High for discovery, docs, and QA planning.
2. Terra Max only for browser QA, where its computer-use performance justifies the middle tier.
3. Sol Medium only for implementation or review that needs that judgment; do
   not route precise bounded implementation to Sol when `fast_implementer`
   applies.
...
Fast model labels are candidates, not routing proof. Use only model IDs exposed by the installed Codex catalog. `fast_implementer` uses Luna High as the incident-driven stable route after Spark caused repeated ownership and instruction-following failures. Keep the mapping provisional until same-packet measurements establish wall time, focused-check pass rate, correction cost, and service-tier cost.
...
- Record each delegated lane's role, agent, harness, resolved model, reasoning,
