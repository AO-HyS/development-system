...
description: Apply PStack-derived tactics when ambiguity, repeated investigation, handoff drift or shared-state contention slows delivery.
---

When intent is ambiguous, restate the problem and observable outcome briefly.
Recover relevant decisions with source pointers. Resolve a consequential unknown
with the smallest executable prototype when this is expected to reduce total
delivery time. Stop exploring once the evidence supports an implementation.
For a new visual direction, use `grill-with-docs` with `design-quality`; preserve
the accepted direction across refinements.
