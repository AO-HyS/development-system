...
For ordinary changes, Astra owns both inspections and the integrated judgment.
...
Inside a Git repository, load `coding-orchestration`. Astra reviews both axes directly for ordinary changes. Use one independent Astra review with separate Standards and Spec notes when security, migrations, cross-cutting changes, or a user request justify independent review. Use two blind reviewers only when their independent investigations materially reduce risk. Resolve providers through the installed roster.
...
presentation; use `pr-lens` for a useful map of the reviewed change.

Generate a `review` technical document from the integrated findings and link
it in the final message, unless the user explicitly opts out:

`development-system document --input packet.json --home HOME --json`

Use the current diff evidence and publication boundaries; state partial or
blocked work as such instead of claiming completeness. The command writes
canonical Markdown and shared-reader HTML under
`HOME/.development-system/private/documents` and grants no workflow authority,
so this already requested local document needs no extra approvals.
