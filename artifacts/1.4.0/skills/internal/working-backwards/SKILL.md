...
Start from the finished customer experience. Accept an ordinary sentence; never require a structured mega-prompt. T3 Code is the conversational and execution surface. HumanLayer is not required.
...
1. `customer-story`: future customer narrative, actor, current pain, promised outcome, first-value journey, external FAQ, internal FAQ, boundaries, and unsupported claims.
2. `research-questions`: only questions about current behavior, code, users, or external facts that could change the product decision.
3. `research-report`: live code/runtime and primary-source answers, with facts, inferences, and unknowns separated.
...
6. `implementation-map`: narrow vertical tickets with outcome, acceptance, checks, dependencies, and one truthful executable frontier.
7. `t3-handoff`: compact private handoff bound to the approved artifacts and exact first slice; `implementationAuthorized: false`.

Create numbered Markdown files with these stems:

```text
01-customer-story.md
02-research-questions.md
03-research-report.md
04-product-contract.md
05-technical-contract.md
06-implementation-map.md
```
...
The renderer supports the safe `flowchart`/`graph` Mermaid subset (`TD`, `TB`, `BT`, `LR`, `RL`) with inline, expanded, and fullscreen reading states; zoom, fit, copy, and source inspection; local bar or line charts from explicit JSON with an accessible data table; semantic Markdown tables and sparse callouts; and fenced code/diff blocks with filename, language, copy, optional line numbers, highlights, wrapping, and clean horizontal scrolling. Prefer a diagram only for graph-shaped relationships and a chart only for real comparative or sequential data. Never fabricate metrics.
