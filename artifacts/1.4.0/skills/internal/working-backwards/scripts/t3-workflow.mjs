 ...
 * @param {{home?: string, workspaceDir: string, repositoryIdentity?: string, repositoryRevision?: string}} input
 */
export async function inspectT3Workflow(input) {
  ...
  await assertSafePrivateWorkspacePath(home, workspaceDir);
  ...
  return {
    ...
    workflowId: id,
    workspaceDir,
    readerPath: resolve(workspaceDir, "index.html"),
...
export async function writeT3Reader(input) {
  ...
  return { ...status, readerWritten: true, localWrites: [status.readerPath] };
}
 ...
 * @param {{home?: string, workspaceDir: string, message: string, repositoryIdentity?: string, repositoryRevision?: string, repositoryPath?: string, now?: () => string}} input
 */
