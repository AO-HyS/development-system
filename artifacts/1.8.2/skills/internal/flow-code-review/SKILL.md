...
Inside a Git repository, load `coding-orchestration`. Astra reviews both axes directly for ordinary changes. Use one Fable review with separate Standards and Spec notes when security, migrations, cross-cutting changes, or a user request justify independent review. Use two blind reviewers only when their independent investigations materially reduce risk. Resolve providers through the installed roster.
