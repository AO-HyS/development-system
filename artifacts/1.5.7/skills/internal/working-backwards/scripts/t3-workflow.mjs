...
const directConversationalApprovalPattern = /^(?:si|yes|claro|perfecto(?:\s+todo(?:\s+aqui)?)?|excelente|listo|correcto|muy bien|todo(?:\s+lo\s+recomendado)?\s+(?:esta\s+)?(?:muy\s+)?bien|todo correcto|de acuerdo|se ve (?:muy )?bien|esta (?:muy )?bien|me gusta(?:\s+como\s+quedo)?|suena (?:muy )?bien|looks good|all good|okay|ok)[.!]*$/iu;
