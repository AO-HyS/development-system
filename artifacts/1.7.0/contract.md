# Development System Contract 1.7.0

Optimize time from an authorized request to a usable result. Correctness is a
set of risk-proportionate constraints; cost breaks ties between effective
routes. Preserve the whole requested outcome through corrections and handoffs.
Do not turn optional process improvements into new product-release gates.
...
editable roster declares the exact model order and reasoning. Factory and
Devin provide fallback execution and Fable assistance for complex independent
review. Luna is the final fast-worker fallback. Sol has no active route.
...
never restart an exhausted provider at every step. Different writers may run
...
a new agent or conversation for each phase. Astra reviews ordinary worker
output. Fable assists on observed complex risk or an explicitly requested
...
Choose focused behavioral tests and static checks from the changed contract.
...
Catalog 0.28.0 includes the Jakub interface skills, make-interfaces-feel-better,
...
Standalone engineering reports use the shared Technical Reader in report mode,
with canonical Markdown and editorial metadata. That presentation creates no
workflow state or implementation authority. Working Backwards keeps its own
workflow presentation. Show-me routes reports to the installed shared helper.
...
Published artifacts and manifests through 1.6.0 remain immutable rollback
