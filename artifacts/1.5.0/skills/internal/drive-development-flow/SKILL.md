...
| In Codex or T3 Code, the user describes a feature, invokes `working-backwards` / `work backwards`, or asks for future customer experience, Amazon Working Backwards, PRFAQ/customer story, or one planning document at a time | `working-backwards` |
...
For implementation work, create one local development-run record at the first active step and append monotonic phase events for discovery, definition, implementation, review, correction, checks, CI, provider waits, agent waits, and human waits as they actually occur. Record the observed harness and resolved model; never write `inherit` as evidence. Overlapping intervals are unioned by the canonical recorder. Missing tokens, cost, quality, CI, or provider telemetry remain `unproven` and never block delivery. Do not ask the operator to repeat the work as an A/B benchmark.
