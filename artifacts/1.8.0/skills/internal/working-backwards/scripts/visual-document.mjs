...
const css = readFileSync(new URL("../assets/visual-document.css", import.meta.url), "utf8");
const font = readFileSync(new URL("../assets/atkinson-hyperlegible-next-latin.woff2", import.meta.url)).toString("base64");
...
export function renderVisualDocument(model, helpers) {
  ...
  const links = model.outline.filter((/** @type {Record<string, any>} */ entry) => entry.level === 2).map((/** @type {Record<string, any>} */ entry) => `<a href="#${esc(entry.id)}">${esc(entry.label)}</a>`).join("");
  const hash = createHash("sha256").update(controller).digest("base64");
  return `<!doctype html><html lang="${language}" data-theme="light"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="generator" content="development-system-technical-reader"><meta name="robots" content="noindex,nofollow"><meta name="referrer" content="no-referrer"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; base-uri 'none'; connect-src 'none'; font-src data:; form-action 'none'; frame-src 'none'; img-src data:; object-src 'none'; script-src 'sha256-${hash}'; style-src 'unsafe-inline'; worker-src 'none'"><title>${esc(doc.title)} · ${esc(model.productName)}</title><style>@font-face{font-family:Reader;src:url(data:font/woff2;base64,${font}) format('woff2');font-weight:100 900;font-display:swap}${css}</style></head><body class="reader-report"><a class="skip-link" href="#document">${language === "es" ? "Ir al documento" : "Skip to document"}</a><header class="brief-topbar"><a class="document-brand" href="#document"><svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M4 3h11l5 5v13H4zM15 3v6h5M8 12h8M8 16h6"/></svg>${esc(model.productName)}</a><details class="document-nav"><summary>${language === "es" ? "En este documento" : "In this document"}</summary><nav>${links}</nav></details><button type="button" class="theme-control" data-theme-toggle aria-label="${language === "es" ? "Cambiar tema" : "Change theme"}" aria-pressed="false"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M20.5 14.2A8.6 8.6 0 0 1 9.8 3.5a8.7 8.7 0 1 0 10.7 10.7Z"/></svg></button></header><main id="document" class="brief"><header class="brief-header"><div><h1>${esc(doc.title)}</h1>${doc.summary ? `<p class="brief-summary">${helpers.inlineMarkdown(doc.summary)}</p>` : ""}</div><aside class="document-facts"><strong>${esc(doc.type)}</strong><p>${esc(doc.status)}</p>${doc.updatedAt ? `<p>${esc(doc.updatedAt)}</p>` : ""}${doc.readTimeMinutes ? `<p>${doc.readTimeMinutes} min ${language === "es" ? "de lectura" : "read"}</p>` : ""}</aside></header><div class="document-body">${content}</div><footer class="brief-footer"><details class="document-source"><summary>${language === "es" ? "Fuente de este documento" : "Document source"}</summary><pre>${esc(doc.markdown || "")}</pre></details><p>${language === "es" ? "Documento local · conserva la evidencia y el alcance de la entrega." : "Local document · preserves delivery evidence and scope."}</p><button type="button" data-print>${language === "es" ? "Imprimir / guardar PDF" : "Print / save PDF"}</button></footer></main><script>${controller}</script></body></html>`;
}
