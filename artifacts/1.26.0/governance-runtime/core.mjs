...
import { describeToolInvocation, classifyToolResult, getAdapterCatalog } from "./adapters.mjs";
...
export function executionDescriptor(run, proposal) {
  ...
  if ((shell.semanticInput.cwd ?? shell.semanticInput.workdir ?? run.root) !== run.root) throw new GovernanceError("Dispatch shell root differs from the governed root", "tool");
...
async function assertBoundarySemantics(run, proposal) {
  ...
  if (proposal.action === "inspect") {
    ...
    const shell = describeToolInvocation({ toolName: proposal.toolName, toolInput: proposal.toolInput });
    assertObservedShellName(run, proposal.toolName);
    if ((shell.semanticInput.cwd ?? shell.semanticInput.workdir ?? run.root) !== run.root) throw new GovernanceError("Inspect root differs from the governed root", "tool");
    const argv = tokenizeExactCommand(shell.semanticInput.command ?? shell.semanticInput.cmd);
...
export async function registerHostSession(input) {
  ...
  await store.withHomeLock(home, async () => {
    ...
    if (prior && prior.cwd === observation.cwd && prior.model === observation.model && prior.reasoning === observation.reasoning && prior.transcriptPath === observation.transcriptPath) observation.observedAt = prior.observedAt;
...
export async function createRun(input) {
  ...
  await store.withHomeLock(home, async () => {
    const registry = await store.readRegistry(home);
    let unfinished = null;
    for (const entry of Object.values(registry.runs)) if (entry.rootSessionId === observation.sessionId || entry.root === root) {
      if (entry.finished !== true || store.hasUnresolvedOwnership(await store.loadRun(entry.runDirectory))) { unfinished = entry; break; }
    }
...
export async function authorizeAction(input) {
  ...
  return mutateRun(runDirectory, async (run) => {
    ...
    if (event.sessionId === run.rootSessionId && (observedModel !== run.coordinator.model || event.reasoning !== run.coordinator.reasoning || !event.cwd || await store.canonicalDirectory(event.cwd) !== run.root)) {
...
export async function bindProcessCandidate(input) {
  return mutateRun(input.runDirectory, async (run) => {
    ...
    attempt.processBinding = { candidateRoot, command: input.command, assessment, baseline: attempt.role === "writer" ? await store.snapshotRepository(candidateRoot) : null };
...
export async function advancePhase(input) {
  ...
  return mutateRun(input.runDirectory, async (run) => {
    ...
    if (isAdjacentForward(run.phase, transition.to, run.taskKind)) {
      ...
      for (const prerequisite of run.taskKind === "audit" && transition.to === "final-review" ? ["plan-review-pass"] : /** @type {Record<string,readonly string[]>} */ (PHASE_PREREQUISITES)[transition.to] ?? []) if (!await prerequisiteSatisfied(run, prerequisite)) throw new GovernanceError(`phase ${transition.to} requires ${prerequisite}`);
...
export async function preflightRun(input) {
  ...
  if (await store.canonicalDirectory(observation.cwd) !== await store.canonicalDirectory(contract.root)) throw new GovernanceError("Contract root differs from the observed session root", "activation", { field: "contract.root" });
  if (await store.readGitHead(contract.root) !== contract.baseSha) throw new GovernanceError("Contract revision differs from current HEAD", "base", { field: "contract.baseSha" });
...
export async function recoverUnstartedRun(input) {
  ...
  return mutateRun(store.runDirectoryFor(home, input.runId), async (run) => {
    ...
    if (!observer || await store.canonicalDirectory(observer.cwd) !== run.root) throw new GovernanceError("Recovery requires a currently observed session at the same root", "activation");
    ...
    run.recoveryReceipts.push({ id: newId("recovery"), originalSessionId: run.rootSessionId, operatorSessionId: input.sessionId, operatorModel: observer.model, operatorReasoning: observer.reasoning, root: run.root, reason: input.reason, at: nowIso(), policyHash: POLICY_HASH });
    for (const permit of run.permits) if (permit.status === "prepared") permit.status = "failed";
...
async function assessmentInput(runDirectory, run, selection) {
  ...
  const candidateHash = stableHash({ root: run.root, baseSha: run.baseSha, epoch: run.acceptanceEpoch, snapshotHash: await store.hashPaths(run.root, paths), sources: await currentSourceHashes(run) });
