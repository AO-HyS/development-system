# Persistent orchestration protocol 1.23.5
...
1.23.5. The inherited policy remains version 1.23.0 and is pinned independently.

## Code-review execution boundary

Packet and integrated code reviewers may execute the required checks with
`danger-full-access` so test-runner temporary files can be created. Product
files, candidate tree, source root, HEAD and branch remain immutable during
review. The controller rejects mutations before acceptance or application.
Planning and visual reviewers retain read-only access. Rejection still requires
concrete findings or an evidence-backed validation blocker; false approval with
empty findings is invalid and cannot be normalized into acceptance.

## Portable planning and correction

Every planning and plan-correction prompt states the existing portability rules:
use relative paths or declared controller placeholders, never the original
source-root path; do not instruct branch changes or commits. Keep the same plan
validator and authoritative worktree binding. A correction that violates these
requirements remains invalid and cannot gain acceptance from the prompt alone.

## Terminal evidence and acceptance progress

Compare repeated acceptance failures by candidate and stable failed criterion
IDs, excluding changing evidence paths from the progress identity. Evidence
file churn cannot establish product progress or waive an acceptance criterion.
Codex success requires a completed native turn and final response in addition
to a zero process exit. Preserve incomplete or interrupted attempts and available
usage. Per-model accounting must expose incomplete usage records consistently
with full-arm accounting; partial known usage never certifies a complete bill.

## Bounded Jev invalid-contract retry

Only a typed malformed-answer response permits an identical-request retry. Each
...
remain hard failures. Keep provider, model, request and validator unchanged;
exhausted attempts fail closed. Persist private per-attempt response
diagnostics and accounting before losing the failed response. Every attempt
belongs in task-active timing and full cost. Retries cannot replace a judgment,
relax a gate or erase failed-run evidence, and incomplete usage remains
explicit.
