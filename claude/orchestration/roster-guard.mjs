...
// model tier (Haiku, Opus low/medium/high, Fable) for implement, plan and review work,
// using the operator's memory of how similar packets went.
...
const rosterList = Object.entries(POLICY.roster).map(([name, r]) => `${name} (${r.model})`).join(', ');
...
function tierLadder(family) {
  const roles = Object.entries(POLICY.roster).filter(([, r]) => family && r.family === family);
...
function memory(family, objective, repo) {
  ...
  const rows = fs.readFileSync(MEMORY, 'utf8').trim().split('\n').slice(-(POLICY.tier.memoryRows ?? 2000))
    .flatMap((line) => { try { const r = JSON.parse(line); return [{ ...r, w: words(r.objective) }]; } catch { return []; } });
  rows.forEach((r, i) => {
...
async function agentCall() {
  ...
  if (!role) {
    ...
    if (type.includes(':')) {
      ...
      deny(`Plugin agent ${type} has no pinned model. Pass model: "opus" (or "haiku" for read-only work) or use a roster role: ${rosterList}.`, entry);
    }
    deny(`"${type}" is not a roster role; it would run on the coordinator's model and effort. Use one of: ${rosterList}. Mapping/search -> code-mapper, docs/web research -> docs-researcher, fully specified edits -> mechanical-worker or exact-implementer, general code -> implementer, one UI screen -> ui-implementer, cross-cutting code -> senior-implementer, decisions/plans -> planner, diff review -> reviewer, image comparison -> visual-reviewer, overall plan or final review -> plan-reviewer, security -> security-reviewer.`, entry);
  }
  if (ti.model && ti.model !== role.model) {
    deny(`${type} runs on ${role.model}; do not override it with "${ti.model}". For more capability dispatch senior-implementer or planner (Opus high); for less, mechanical-worker or code-mapper (Haiku).`, entry);
  }
