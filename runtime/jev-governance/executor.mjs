...
import { bindProcessCandidate, executionDescriptor, getRun, recordHostEvent, snapshotPaths, stableHash } from "./core.mjs";
...
export async function launchGovernedProcess({ home, runDirectory, sessionId, launch, inputPath, signal }) {
  if (!launch || typeof launch !== "object" || Array.isArray(launch)
    || Object.keys(launch).some((key) => !["attemptId", "candidateRoot", "packetPath", "check"].includes(key))
  ...
  try {
    ...
    const argv = tokenizeExactCommand(permit.toolInput?.command);
    if (permit.toolName !== "Bash" || !argv || !["node", process.execPath].includes(argv[0]) || argv[1] !== cliPath || argv[2] !== "execute") invalid("The consumed permit is not an exact attached governance execution.");
    ...
    const processArgs = check ? launch.check.argv : flash ? flashArguments(packetPath, candidateRoot, prompt)
      : ["exec", "--json", "--sandbox", "read-only", "--model", proposal.route.model, "-c", `model_reasoning_effort="${proposal.route.reasoning}"`, "--cd", candidateRoot, prompt];
