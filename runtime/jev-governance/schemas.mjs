...
/** Raised for every rejected input or invariant violation in this runtime. */
...
export function assertRecord(value, label) {
  if (!isRecord(value)) throw new GovernanceError(`${label} must be an object`);
}

/** @param {unknown} value @param {string} label */
export function assertSafeId(value, label) {
  if (typeof value !== "string" || !SAFE_ID.test(value)) throw new GovernanceError(`${label} must be a safe identifier`);
}
...
function validateTicketsAndCriteria(tickets, criteria) {
  ...
  const normalizedCriteria = criteria.map((/** @type {any} */ criterion, /** @type {number} */ index) => {
    ...
    rejectUnknownKeys(criterion, ["id", "ticketId", "requirement", "evidenceRequired"], `contract.criteria[${index}]`);
    ...
    return { id: criterion.id, ticketId: criterion.ticketId, requirement: criterion.requirement.trim(), evidenceRequired: criterion.evidenceRequired.trim() };
  });
  for (const ticket of normalizedTickets) {
...
export function validateContract(input, policy) {
  ...
  rejectUnknownKeys(input, ["id", "root", "baseSha", "endpoint", "authorization", "sources", "tickets", "criteria", "capacity"], "contract");
...
export function validateProposal(input) {
  ...
  if (!isNonEmptyString(input.phase)) throw new GovernanceError("proposal.phase is required");
  if (!isNonEmptyString(input.action)) throw new GovernanceError("proposal.action is required");
  ...
  if (!isNonEmptyString(input.objective)) throw new GovernanceError("proposal.objective is required");
  if (!isStringArray(input.requirementIds)) throw new GovernanceError("proposal.requirementIds must be a string array");
  if (!isStringArray(input.sourceIds)) throw new GovernanceError("proposal.sourceIds must be a string array");
  if (!isStringArray(input.readSet) || !isStringArray(input.writeSet)) throw new GovernanceError("proposal readSet and writeSet must be string arrays");
  ...
  if (!isStringArray(input.dependsOn)) throw new GovernanceError("proposal.dependsOn must be a string array");
  ...
  if (!isNonEmptyString(input.toolName)) throw new GovernanceError("proposal.toolName is required");
  if (!isRecord(input.toolInput)) throw new GovernanceError("proposal.toolInput must be an object");
  return {
    id: input.id,
...
export function validateTransition(input) {
  ...
  if (!isNonEmptyString(input.to)) throw new GovernanceError("transition.to is required");
  if (!isNonEmptyString(input.reason)) throw new GovernanceError("transition.reason is required");
...
export function validateOutcome(input) {
  ...
  if (!["accepted", "blocked", "interrupted"].includes(input.status)) throw new GovernanceError("outcome.status must be accepted, blocked or interrupted");
  if (!isNonEmptyString(input.reason)) throw new GovernanceError("outcome.reason is required");
...
export function validateHostEvent(input) {
  ...
  if (input.kind === "process-start" || input.kind === "process-exit") {
    rejectUnknownKeys(input, ["kind", "attemptId", "attempt_id", "processId", "process_id", "provider", "model", "reasoning", "exitCode", "exit_code", "terminated", "cancelled", "output", "changedPaths", "changed_paths", "candidateRoot", "processSessionId", "command"], "event");
    ...
    if (input.kind === "process-exit") {
      if (!Number.isSafeInteger(exitCode)) throw new GovernanceError("event.exit_code must be an integer");
