 ...
 * }} input
 * @returns {Promise<{ok:true, obligations:Array<{id:string,outcome:string,confidence:number}>, route:any, tool:any, priority:any, usage:any, model:string, requestHash:string, requestBytes:number, latencyMs:number} | {ok:false, reason:string}>}
 */
export async function classifyWithJev(input) {
  ...
  if (run.criteria.length > JEV_MAX_CRITERIA) return { ok: false, reason: "criteria exceed the classifier cap" };
  ...
  for (const source of run.sources.filter((/** @type {any} */ entry) => proposal.sourceIds.includes(entry.id))) {
    ...
    if (excerpt === null) return { ok: false, reason: "a required source is missing or exceeds the complete-source byte cap" };
  ...
  if (obligations.some((id) => ["changed_scope", "repo_patterns", "docs_consistency"].includes(id))) {
    ...
    } catch { return { ok: false, reason: "required returned-code context is missing, unsupported or exceeds the complete-context cap" }; }
  }
  ...
  if (requestBytes > JEV_MAX_REQUEST_BYTES) return { ok: false, reason: "request exceeds the classifier byte cap" };

  let apiKey;
  try {
    apiKey = await loadApiKey({ home: input.home, env: input.env });
  } catch {
    return { ok: false, reason: "private Jev credential is unavailable" };
  }
  if (!apiKey) return { ok: false, reason: "TYPESAFE_API_KEY is not configured" };

  const endpoint = JEV_ENDPOINT;
  if (body.includes(apiKey)) return { ok: false, reason: "protected credential data appeared in classifier context" };
  ...
  try {
    ...
    const reason = error instanceof GovernanceError ? error.message : "Jev transport failed";
    return { ok: false, reason };
  }
  try {
    const validated = validateJevResponse(payload, obligations, candidates, applicability);
    if (JSON.stringify(payload).includes(apiKey)) return { ok: false, reason: "protected credential data appeared in classifier output" };
    return { ok: true, ...validated, requestHash: stableHash(body), requestBytes, latencyMs: Date.now() - started };
  } catch (error) {
    return { ok: false, reason: error instanceof GovernanceError ? error.message : "Jev response was malformed" };
  }
}
