...
The canonical, versioned source for Alejandro Ortiz Corro's global development contract. Version `1.5.18` preserves the executable anti-slop protocol from 1.5.17 and makes its live capability probes reliable with catalog `0.25.0`: canonical behavior signatures now match equivalent terms actually produced from the installed instructions, while prompts still contain none of their own signatures and exact installed-file read proof, catalog discovery, and substantive skill-specific answers remain mandatory. The catalog keeps Dillon Mulroy's upstream `dmmulroy/anti-slop` install skill at exact commit `e8c4880471b23ab7f216fba7b27d173a6ef07d4c` with MIT provenance behind the contained Development System installer; it preserves the fast chain, evidence-bound adversarial review fallback, and opt-in product verification. The five core repositories are NutriPlan, The Barber Central, Casa Roca, aohys.com, and ETERIA.
...
./bin/development-system install --version 1.5.18
./bin/development-system sync-skills --version 0.25.0
./bin/development-system guardrails-enable
pnpm run skills:probe
./bin/development-system audit-skills --version 0.25.0 --evidence "$HOME/.development-system/private/reports/skills-live-latest.json"
...
`model-route` is a pure resolver. For adversarial review it tries Factory Fable
5.1, Devin Fable 5.1, then Codex Sol, recording every quota/provider boundary;
it never calls a provider. Fable and Sol use `xhigh` by default, while `max`
requires explicit escalation. The ordinary implementation route and all
mechanical, non-deliberative work follow one shared ordered runtime attempt
chain: Devin `swe-1-7`, Factory `glm-5.3-flash`, Devin `gemini-3.8-flash` only
with current runtime evidence, then Codex `gpt-5.6-luna` with reasoning `max`
on the priority/fast service path. The parent resolves and attempts the route
before dispatch and requires a provider/runtime receipt before claiming the
resolved model; the Codex `fast_implementer` runs only when the Luna fallback
is selected. Everyday implementation is never Codex Luna High. The same route
owns trivial edits, code/file search, file creation, code generation, evidence
collection, and focused tests. Runtime receipts measure actual latency and
correctness without changing the user-declared order; the Sol parent retains
...
- “Instala la versión actual del sistema de desarrollo” → `install --version 1.5.18`, `sync-skills --version 0.25.0`, then `guardrails-enable`
