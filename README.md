...
The canonical, versioned source for Alejandro Ortiz Corro's multi-harness development contract. Version `0.2.0` adds a pinned, reversible skill catalog and operational Codex/Factory evidence; T3Code consumes the Codex surface.
...
node ./bin/development-system.mjs install --version 0.2.0
...
The scenario creates isolated temporary HOMEs. It proves the original contract install/drift/reinstall/rollback path, then seeds a stale workspace and broken skill link, synchronizes authoritative skill sources, verifies mirrors and loadability, detects drift, reinstalls, rolls back exact prior entries, and preserves unrelated user files. It never touches the real HOME.

## Natural-language recovery

No secret phrase is required. Requests such as these map to the same explicit operations:

- “Instala la versión 0.2.0 del sistema de desarrollo” → `install --version 0.2.0` plus `sync-skills --version 0.2.0`
...
Contract versions use semantic versioning. `0.0.0` is the bootstrap rollback target, `0.1.0` is the first canonical contract, and `0.2.0` is the skill-catalog contract. Published manifests and artifacts are immutable. The installed manifest resolves `$INSTALL_COMMIT` to the exact commit used for installation; upstream skills pin the exact authoritative commit directly.

## Current boundary

This slice proves skill provenance, reversible synchronization, catalog health, mirrors/adapters, and real Codex/Factory loading influence for a tracer skill. Lifecycle gates, model benchmarks, delivery review, repository normalization, and product pilots remain later modules.
