...
import { buildTechnicalReaderModel, renderTechnicalReaderHtml } from '../artifacts/1.29.0/skills/internal/working-backwards/scripts/t3-reader.mjs';
import { writeTechnicalReport } from '../artifacts/1.29.0/skills/internal/working-backwards/scripts/t3-report.mjs';
import { startReaderLive } from '../artifacts/1.29.0/skills/internal/working-backwards/scripts/reader-live.mjs';
...
test('Reader server stores margin questions as revisioned batches and refuses unsafe writes', async () => {
  ...
  try {
    ...
    const escapedEndpoint = endpoint.replace('report.questions.json', encodeURIComponent(join(workspaceDir, 'report.questions.json')));
    assert.equal((await fetch(escapedEndpoint, { method: 'POST', headers: { 'Content-Type': 'application/json', Origin: origin }, body: JSON.stringify({ expectedRevision: 0, data }) })).status, 404);
    assert.equal((await fetch(endpoint.replace('report.questions.json', 'folder%5creport.questions.json'))).status, 404);
    await assert.rejects(stat(join(workspaceDir, 'report')), { code: 'ENOENT' });
