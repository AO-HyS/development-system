...
test("1.5.12 routes ordinary work directly and keeps Working Backwards opt-in", async () => {
  const [manifest, catalog, contract, packageJson] = await Promise.all([
    readFile(resolve(root, "manifests/1.5.12.json"), "utf8").then(JSON.parse),
    readFile(resolve(root, "catalog/0.19.0.json"), "utf8").then(JSON.parse),
    readFile(resolve(root, "artifacts/1.5.12/contract.md"), "utf8"),
    readFile(resolve(root, "package.json"), "utf8").then(JSON.parse),
  ]);
  assert.equal(manifest.contractVersion, "1.5.12");
  assert.equal(catalog.catalogVersion, "0.19.0");
  assert.equal(packageJson.version, "1.5.12");
  ...
  assert.equal(drive.source.path, "artifacts/1.5.12/skills/internal/drive-development-flow");
  assert.equal(orchestration.variants[0].sourceDirectory, "artifacts/1.5.12/skills/internal/coding-orchestration");
  assert.equal(orchestration.source.path, orchestration.variants[0].sourceDirectory);
  assert.equal(orchestration.variants[0].adapterContract, "fast-model-first-orchestration-v5");
...
test("published 1.5.11 contract and catalog remain distinct immutable sources", async () => {
  const [oldContract, oldCatalog, nextContract, nextCatalog] = await Promise.all([
    ...
    readFile(resolve(root, "artifacts/1.5.12/contract.md"), "utf8"),
    readFile(resolve(root, "catalog/0.19.0.json"), "utf8"),
  ]);
