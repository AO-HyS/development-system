...
test("catalog 0.8.0 installs the progressive T3 Code workflow without claiming live evidence", async () => {
  const catalog = JSON.parse(await readFile(resolve(repositoryRoot, "catalog", "0.8.0.json"), "utf8"));
  ...
  assert.deepEqual(skill.variants[0].executableFiles, ["scripts/t3-workflow.mjs"]);
  const drive = catalog.skills.find((entry) => entry.logicalName === "drive-development-flow");
  assert.equal(drive.source.path, "artifacts/1.4.0/skills/internal/drive-development-flow");
  assert.equal(catalog.operationalEvidenceSkills.includes("working-backwards"), false);
});

test("contract 1.4.0 and catalog 0.8.0 install into an isolated HOME", async () => {
  const home = await mkdtemp(resolve(tmpdir(), "aohys-wb-home-"));
  const sourceCommit = await candidateCommit();
  for (const args of [
    ["install", "--version", "1.4.0", "--source-commit", sourceCommit],
    ["sync-skills", "--version", "0.8.0", "--source-commit", sourceCommit],
  ]) {
  ...
  assert.equal(installed.contractVersion, "1.4.0");
  ...
  assert.match(await readFile(resolve(home, ".agents", "skills", "working-backwards", "scripts", "t3-workflow.mjs"), "utf8"), /renderTechnicalReader/);
  assert.match(await readFile(resolve(home, ".agents", "skills", "working-backwards", "scripts", "t3-reader.mjs"), "utf8"), /buildTechnicalReaderModel/);
  assert.match(await readFile(resolve(home, ".agents", "skills", "working-backwards", "SKILL.md"), "utf8"), /shared technical Reader/i);
  assert.doesNotMatch(await readFile(resolve(home, ".agents", "skills", "working-backwards", "SKILL.md"), "utf8"), /Use HumanLayer as the surface/i);
  assert.equal(
