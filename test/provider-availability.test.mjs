...
test("a host quota failure skips a candidate only until its expiry", async () => {
  ...
  const observation = { candidateId: "opencode-muse-spark-1.3-contributor", reason: "quota-exhausted", observedAt: new Date(now).toISOString(), expiresAt: new Date(now + 60000).toISOString(), evidenceRef: "private/provider-response.json" };
  try {
    ...
    assert.equal(route(await readProviderFailures(home, now)).selected.model, "opencode-go/glm-5.3-flash");
    assert.equal(route(await readProviderFailures(home, now + 60000)).selected.model, "opencode-go/muse-spark-1.3-contributor");
