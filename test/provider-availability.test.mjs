...
test("a host quota failure skips a candidate only until its expiry", async () => {
  ...
  const observation = { candidateId: "codex-terra-priority", reason: "quota-exhausted", observedAt: new Date(now).toISOString(), expiresAt: new Date(now + 60000).toISOString(), evidenceRef: "private/provider-response.json" };
  try {
    ...
    assert.equal(route(await readProviderFailures(home, now)).selected.model, "gpt-5.6-sol");
    assert.equal(route(await readProviderFailures(home, now + 60000)).selected.model, "gpt-5.6-terra");
