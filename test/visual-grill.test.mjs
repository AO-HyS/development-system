...
test("current release retains visual quality skills, updates governed intake, and pins its Astra XHigh visual reviewer with explicit hashes", async () => {
  ...
  const manifest = JSON.parse(readFileSync(resolve(repositoryRoot, "manifests/1.26.0.json"), "utf8"));
  ...
  assert.equal(packageJson.contractVersion, "1.26.0");
  assert.equal(manifest.contractVersion, "1.26.0");
