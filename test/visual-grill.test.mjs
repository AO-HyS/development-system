...
test("current release retains visual quality skills, updates governed intake, and pins its Astra XHigh visual reviewer with explicit hashes", async () => {
  ...
  const manifest = JSON.parse(readFileSync(resolve(repositoryRoot, "manifests/1.26.1.json"), "utf8"));
  ...
  assert.equal(packageJson.contractVersion, "1.26.1");
  assert.equal(manifest.contractVersion, "1.26.1");
  ...
  for (const name of ["drive-development-flow", "grill-with-docs", "design-direction", "design-quality"]) {
    ...
    const version = ["drive-development-flow", "grill-with-docs"].includes(name) ? "1.26.0" : "1.22.1";
