...
test("current release retains the four 1.22.1 canonical skills and visual reviewer with explicit hashes", async () => {
  ...
  const manifest = JSON.parse(readFileSync(resolve(repositoryRoot, "manifests/1.23.2.json"), "utf8"));
  ...
  assert.equal(packageJson.contractVersion, "1.23.2");
  assert.equal(manifest.contractVersion, "1.23.2");
