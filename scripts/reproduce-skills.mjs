...
await cp(
  resolve(repositoryRoot, "artifacts", "1.5.8"),
  resolve(sourceRoot, "artifacts", "1.5.8"),
  { recursive: true },
);
for (const args of [
  ["init"],
...
assert.equal(structurallyHealthy.logicalSkillCount, 34);
assert.equal(structurallyHealthy.physicalVariantCount, 34);
