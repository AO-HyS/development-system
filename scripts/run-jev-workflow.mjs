...
export async function runWorkflow(config, runtime = {}) {
  ...
  const jevPrices = await readJson(fileURLToPath(new URL('../config/1.23.0/api-prices.json', import.meta.url)));
  ...
  async function judge(atom, boundary, candidate) {
    ...
    for (const packet of packets) {
      const input = structuredClone({ atom: { ...atom, exactContext: undefined, observedFacts: { boundary, candidateHash: candidate?.hash ?? null } },
        ...
        contextMode: /** @type {'flow' | 'compact'} */ (packet.flowContext ? 'flow' : 'compact'), flowContext: packet.flowContext });
      /** @type {string | null} */ let requestHash = null;
      /** @type {typeof fetch} */
      const fetchExact = async (url, options) => {
        if (typeof options?.body !== 'string') throw new Error('Jev request must have a serialized body');
        const nextHash = digest(options.body);
        if (requestHash !== null && requestHash !== nextHash) throw new Error('Jev retry request changed');
        requestHash = nextHash;
        return (runtime.fetchImpl ?? fetch)(url, options);
      };
      for (let attempt = 1; attempt <= 3; attempt += 1) {
        const startedAt = new Date().toISOString();
        const receiptPath = resolve(directory, 'jev', `${atom.id}-${boundary}-${randomUUID()}.json`);
        /** @type {string | null} */ let rawResponsePath = null;
        /** @type {number | null} */ let httpStatus = null;
        /** @type {any} */ let receipt;
        /** @type {unknown} */ let failure;
        let valid = false;
        try {
          receipt = await classifyAtomWithJev({ ...input, apiKey, fetchImpl: fetchExact, onResponse: async diagnostic => {
            if (diagnostic.requestHash !== requestHash) throw new Error('Jev response request hash mismatch');
            httpStatus = diagnostic.httpStatus;
            const path = receiptPath.replace(/\.json$/, '.response.json');
            await json(path, diagnostic.body);
            rawResponsePath = path;
          } });
          valid = true;
        } catch (error) { failure = error; }
        const facts = receipt ?? { model: null, usage: null, usageComplete: false, ...(failure instanceof JevClassificationError ? failure.telemetry : {}) };
        const usageComplete = facts.usage && ['input_tokens', 'output_tokens'].every(key => Number.isSafeInteger(facts.usage[key]) && facts.usage[key] >= 0);
        const rates = jevPrices.models['jev-1.13.0'];
        const apiEquivalentCostUsd = usageComplete && facts.model === 'jev-1.13.0'
          ? (facts.usage.input_tokens * rates.input + facts.usage.output_tokens * rates.output) / 1e6 : null;
        const summary = { atomId: atom.id, boundary, startedAt, endedAt: new Date().toISOString(), attempt, valid,
          model: facts.model, usage: facts.usage, usageComplete: Boolean(usageComplete), requestHash, requestBytes: facts.requestBytes,
          latencyMs: facts.latencyMs, questions: packet.flowContext ? 8 : 5, coveredFiles: packet.path ? [packet.path] : [],
          apiEquivalentCostUsd, apiCostComplete: apiEquivalentCostUsd !== null, pricingVersion: jevPrices.version,
          httpStatus, rawResponsePath, error: valid ? null : failure instanceof Error ? failure.message : 'Jev failure' };
        await json(receiptPath, { ...facts, ...summary, reviewCoverage: packet.flowContext?.reviewCoverage ?? [] });
        await event('jev-completed', { ...summary, receiptPath });
        if (!valid) {
          if (failure instanceof JevClassificationError && failure.retryableInvalidAnswer) {
            if (attempt < 3) continue;
            throw new Error(`Jev invalid answer contract after 3 total attempts: ${failure.message}`, { cause: failure });
          }
          throw failure;
        }
  ...
  try {
    ...
    await json(resolve(directory, 'manifest.json'), { ...config, ticket, acceptanceIds, requiresVisualReview, planningMode, packetization, concurrency: capacity, coordinator, startedAt: new Date().toISOString(), controllerVersion: '1.23.5' });
    ...
    for (;;) {
      ...
      if (!qa.accepted) {
        const failedIds = acceptanceIds.filter((/** @type {string} */ id) => qa.criteria.some((/** @type {any} */ criterion) => criterion.id === id && !criterion.passed));
        requireProgress('acceptance', before, failedIds);
