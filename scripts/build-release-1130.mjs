...
const catalog = await read("catalog/0.33.0.json");
catalog.catalogVersion = catalogVersion;
catalog.skills.push({ logicalName: "design-direction", physicalHarnesses: ["codex"],
  source: { repository: "https://github.com/AO-HyS/development-system", commit: "$INSTALL_COMMIT", path: "" },
  variants: [{ id: "design-direction.codex", harness: "codex", sourceDirectory: "", destination: ".agents/skills/design-direction", expectedMirrorOf: null, folderSha256: "" }] });
for (const name of ["design-direction", "drive-development-flow", "design-quality", "grill-with-docs"]) {
...
const manifest = await read("manifests/1.12.0.json");
