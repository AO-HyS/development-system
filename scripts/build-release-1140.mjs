...
const catalog = await read("catalog/0.34.0.json");
catalog.catalogVersion = catalogVersion;
for (const name of ["coding-orchestration", "impeccable"]) {
  const skill = catalog.skills.find((/** @type {{logicalName: string}} */ item) => item.logicalName === name);
  const directory = `artifacts/${version}/skills/${name === "impeccable" ? "upstream" : "internal"}/${name}`;
  skill.source.path = directory;
  if (name === "impeccable") skill.source.upstreamReference = { repository: "https://github.com/pbakaus/impeccable", commit: "cd12f8660e2dde57b9615c8a6b8ea674101f9cfc", version: "4.3.1", path: "universal.zip/.agents/skills/impeccable", license: "Apache-2.0" };
...
const manifest = await read("manifests/1.13.0.json");
...
manifest.artifacts.push({ id: "personal-codex-instructions.codex", logicalName: "personal-codex-instructions", sourcePath: `artifacts/${version}/global-codex-instructions.md`, destination: ".codex/AGENTS.md", harness: "codex", sha256: hash(await readFile(resolve(root, `artifacts/${version}/global-codex-instructions.md`))), expectedMirrorOf: null });
