...
const catalog = await read("catalog/0.27.0.json");
catalog.catalogVersion = catalogVersion;
for (const name of ["working-backwards", "show-me"]) {
  ...
  const upstreamReference = name === "show-me" ? skill.source : null;
  ...
  for (const variant of skill.variants) {
    ...
    if (name === "working-backwards") variant.executableFiles = [...variant.executableFiles, "scripts/t3-report.mjs"];
  }
}
...
const manifest = await read("manifests/1.6.0.json");
