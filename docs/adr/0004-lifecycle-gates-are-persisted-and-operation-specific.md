...
Implement Preview records one terminal slice and grants only its delivery operations. Merge, release, production, paid activation, and destructive operations use exact, one-shot authorizations that are available only after pre-release evidence. A denied or out-of-order request does not persist state or report external side effects.
