...
Every prompt makes three product implementation workstreams explicit instead of hiding them inside a generic audit:

1. **Product architecture** — module ownership, React/TanStack composition, strict types, discoverability, and fast product-level fitness functions.
...
Agent guardrails, global anti-slop policy, and Release Train design are Development System capabilities. Product convergence verifies only that each repository exposes compatible commands and adapter metadata; it does not create product architecture workstreams or tickets to redesign those global capabilities.
